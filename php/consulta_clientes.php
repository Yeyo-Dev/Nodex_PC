...
if ($totCuentas>0) {
		...
		while ($rowCunt = mysqli_fetch_array($res)) {
			...
			echo '<button onclick="modificar( \'clientes\', \''.$rowCunt['clientes_id'].'\')" >Modificar</button><button onclick="eliminar(\'clientes\',\'clientes_id\',\''.$rowCunt['clientes_id'].'\')">Eliminar</button>';
			echo '</div>';
		}
