...
if (empty($buscar) || $buscar=""){
	$sql = "SELECT * FROM pedidos ORDER BY nombre ASC";
}else{
	$sql = $busqueda;
}
...
if ($totCuentas>0) {
		while ($rowCunt = mysqli_fetch_array($res)) {
			...
			echo '<button>Modificar</button><button onclick="eliminar("pedidos","pedido_folio",'.$rowCunt['pedido_folio'].')">Eliminar</button>';
            echo '</div>';
		}
